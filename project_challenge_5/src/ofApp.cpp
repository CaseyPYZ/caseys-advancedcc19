...
void ofApp::setup(){
    ...
    // right arm
    ruaPos = ofVec2f(rElbow.x, rElbow.y);
    rlaPos = ofVec2f(rElbow.x-rLAs.x, rElbow.y);
    
    // left arm
    luaPos = ofVec2f(lShoulder.x, lShoulder.y);
    llaPos = ofVec2f(lElbow.x,lElbow.y);
    
    // right leg
    rulPos = ofVec2f(rHip.x, rHip.y);
    rllPos = ofVec2f(rKnee.x, rKnee.y);
    
    // left leg
    lulPos = ofVec2f(lHip.x, lHip.y);
    lllPos = ofVec2f(lKnee.x, lKnee.y);
...
void ofApp::draw(){
    ...
    headPNG.draw(neck.x-heads.x/2, neck.y-heads.y/2, heads.x, heads.y);
...
//    ofPushMatrix();
//    ofTranslate(rShoulder.x, rShoulder.y);
//    ofDrawCircle(0,0,50);
...
//    ruaPNG.draw(0, 0, rUAs.x, rUAs.y);
//
//    ofPopMatrix();
    
    ofLogNotice() << "up pressed" << endl;
}
